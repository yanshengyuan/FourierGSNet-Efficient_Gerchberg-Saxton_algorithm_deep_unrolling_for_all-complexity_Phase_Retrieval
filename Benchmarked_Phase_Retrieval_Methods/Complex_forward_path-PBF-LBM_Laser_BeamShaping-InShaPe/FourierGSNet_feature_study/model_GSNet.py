...
class FFT_layer(nn.Module):
    ...
    def forward(self, source_phase, pupil_field, Fourier_intensity, pupil_intensity, epoch=None, num_batch=None, num_layer=None, val=False, output_dir=None):
        ...
        Fourier_real_vis = Fourier_real.squeeze().detach().cpu().numpy()[0]
        Fourier_imag_vis = Fourier_imag.squeeze().detach().cpu().numpy()[0]
        Fourier_real_vis = normalize(Fourier_real_vis)
        Fourier_imag_vis = normalize(Fourier_imag_vis)
        plt.imshow(Fourier_real_vis, cmap='twilight_shifted')
        plt.axis('off')
        plt.tight_layout(pad=0)
        plt.savefig('Fourier_real.png')
        plt.close()
        plt.imshow(Fourier_imag_vis, cmap='twilight_shifted')
        plt.axis('off')
        plt.tight_layout(pad=0)
        plt.savefig('Fourier_imag.png')
        plt.close()
...
class GSNet_Fourier(nn.Module):
    ...
    def forward(self, source_intensity, image_intensity, epoch=None, num_batch=None, val=False, output_dir=None):
        if(val==True):
            ...
            generation_folder = os.path.join(output_dir, str(epoch), "phase_gen")
...
class Condition_UNet(nn.Module):
    ...
    def forward(self, x, condition, attention, epoch=None, num_batch=None, num_layer=None, val=False, output_dir=None):
        ...
        b, g, c, h, w = attention_map.shape
        c_sqrt = int(np.sqrt(c))
        attention_map = rearrange(attention_map, "b c (g1 g2) h w -> b c (g1 h) (g2 w)", g1=c_sqrt, g2=c_sqrt)
        ...
        if(val==True):
            for i in range(out.shape[0]):
                heat_map=out[i].squeeze().cpu().numpy()
                feature_img_name = "layer"+str(num_layer)+"_batch"+str(num_batch)+"_"+str(i)+".png"
                path = os.path.join(generation_folder, feature_img_name)
                plt.imsave(path , heat_map, cmap='gray')
