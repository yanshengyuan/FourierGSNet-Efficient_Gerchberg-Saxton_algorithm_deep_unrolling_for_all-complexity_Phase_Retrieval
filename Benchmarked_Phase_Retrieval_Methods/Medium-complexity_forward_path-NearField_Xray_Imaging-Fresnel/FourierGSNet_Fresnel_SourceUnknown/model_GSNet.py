...
class FFT_layer(nn.Module):
    ...
    def forward(self, source_phase, pupil_field, Fourier_intensity, epoch=None, num_batch=None, num_layer=None, val=False, output_dir=None):
        ...
        pupil_intensity = torch.clamp(pupil_intensity, 0.0, 2.0)
        pupil_amp = torch.sqrt(pupil_intensity)
...
class GSNet_Fourier(nn.Module):
    def __init__(self, gen_phase_init, gs_phase_init, gs_intensity_init, args, num_layers=10):
        ...
        self.gs_intensity_init = gs_intensity_init
    ...
    def forward(self, image_intensity, epoch=None, num_batch=None, val=False, output_dir=None):
        if(val==True):
        ...
        source_amp = torch.sqrt(self.gs_intensity_init)
        init_field = source_amp*torch.exp(self.gs_phase_init * 1j)
        gen_phase_init = self.gen_phase_init.unsqueeze(1)
        F, phase = self.input_layer(gen_phase_init, init_field, image_intensity, epoch, num_batch, 0, val, output_dir)
        cnt = 1
        for layer in self.layers:
            F, phase = layer(phase, F, image_intensity, epoch, num_batch, cnt, val, output_dir)
            cnt += 1
