...
from utils.loader import  get_training_data,get_validation_data
from model_ICLR import GSNet_FFT as PImodel
...
parser.add_argument('--gridsize', default=256*1e-6, type=float)
parser.add_argument('--wavelength', default=1e-10, type=float)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    train_dataset = get_training_data(traindir)
    ...
    H=0
    W=0
    for i, (I, Phi, lightsource) in enumerate(train_loader):
        B, C, H, W = Phi.shape
        print(H, W)
        break
    if(H!=W):
    ...
    imag = defocus_kernel.imag.astype(np.float32)
    defocus_phase_mask = np.arctan2(imag, real)
    plt.imsave("defocus_phase_mask.png", defocus_phase_mask, cmap='gray')
    
    defocus_phase_mask = torch.tensor(defocus_phase_mask).cuda(args.gpu, non_blocking=True)
    model = PImodel(Phi_init, defocus_phase_mask, Intensity_init, args)
    print(model)
...
def train(train_loader, model, criterion, optimizer, epoch, device, args):
    ...
    for i, (I, Phi, lightsource) in enumerate(train_loader):
        ...
        I_far = I.to(device, non_blocking=True).squeeze()
        Phi=Phi.squeeze()
